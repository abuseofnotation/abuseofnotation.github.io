...
You can see that this block now starts to look like a piece of logic, as opposed to a random pile of conditionals. We can now reason about it, for example we can point out that all combinations of values of `a` and `b` are handled (but more on that later). In short, our code does not work like a Rube Goldberg machine any more.
