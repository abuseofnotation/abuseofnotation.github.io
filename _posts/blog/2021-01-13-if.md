...
Let's see one more example of how we can utilize data structures. This one has to do with error handling. We often have multiple ways in which things can go wrong and we are encoding them with `if`s like this.
...
That makes it easy to gather all the errors and return them as an array:
