---
category: blog
title: Structured programming with `if` statements
layout: blog
---

The `if` statement (or `if` expression) is the cornerstone of every modern programming language - it is so pervasive that we rarely think about how exactly should we use it, or how it is meant to be used. But despite its popularity, `if` wasn't always there, and it wasn't as pervasive as nowadays, so its role is, I'd argue, still somewhat misunderstood, in this article, I'd examine some mistakes that we can easily avoid in order to improve on our code.
<!--more-->

Before we begin: My perspective on programming
===

This is an opinionated text, i.e. it contains my opinions and although I have many arguments that back them, they are ultimately based on my core beliefs about what programming is and should be. So here are some of the things on which this advice is based on, structured as postulates.

> 1. Code is for humans (and programmers are humans too).
...
> 2. Code should be as bug-free as possible (and easy to debug as well).

This one looks pretty self-explanatory, but the key point is that we have to dedicate effort to make it such, by constantly thinking not only about ways in which the code might go wrong, but also about ways to diagnose potential problems. Also, there is the 
...
if (input less than waterleve) {
  turn motor on
} else if (input less than waterleve) {
turn motor off
} else {
...
```

The adoption of structured programming was a long process which began with people even questioning whether structured programs are even capable of encoding all possible computations, but today, although some people are [still defending the use of goto](https://stackoverflow.com/questions/46586/goto-still-considered-harmful), it (the process) is largely finished - `goto` is banished and the structured style of programming is so pervasive across modern languages that the term is not used any more, simply because there is nothing to contrast it with. 
...
When used like this, `return` is direcly translatable to a `goto`.
...
but people would prefer having the main function code directly in the body in order to avoid indentation (especially if there is a lot of it already). According to them (and they have a point) the error handling is somewhat outside of the domain of the function, we can perhaps say that they are at different levels of abstraction. In cases like this, we should use *functions* to structure our code and to keep the different levels of abstraction separate. Functions are everything that random sequences of statement aren't - they *have names*, they have *their own scope*, and most importantly they are *easy to debug* (e.g. when using separate function, I'd have the name of the block that went wrong right there on the top of the stack trace. So let's look at how the code above can be made to work with functions:
...
I think this is a very cool and underused pattern that allows you to lose the indentation without sacrificing your structure.

Rule 2 - DO NOT conflate multiple logical conditions in a single statement 
===

This is another bad pattern that, we might say, has the same root cause as the first one - in their effort to keep things simple and concise, people don't include all the structure that needs to be there in a robust piece of code. In `if` statements we often have to deal with multiple conditions that are dependent on one another. In such cases, people try to deal with all of them in one statement with multiple branches and `else if`-s. I argue that it's actually better to have *one statement per condition* and to nest statements to get the control flow we want. An example is worth a thousand words here:
...
You can see that this block now starts to look like a piece of logic, as opposed to a random pile of conditionals. Now we can reason about it, for example we can point out that all combinations of `a` and `b` are handled (but more on that later). In short, our code does not work like a Rube Goldberg machine any more.

Some people would argue that having to repeat the `do the other thing` block in two branches is a deficiency , but I beg to disagree - the two branches represent two quite different cases and the fact that we handle those two cases in the same way is hardly a reason for us not to enumerate them explicitly. This is one of those cases where we lose a little bit of conciseness but we gain a great deal in terms of readability and robustness.
...
We all know that `if` statements should handle all possible cases and doing so is very easy, we keep one condition per statement and we ensure that every `if` statement has an `else` clause which would handle all cases where the other condition or conditions are not satisfied. Although simple, I've seen a couple of cases where this rule is broken. What do we do, for example, when there is nothing that we *should* do in a given case? Imagine, for example, that we have a "Sing Up" button that can be disabled and that does not work if it is disabled. The specification might say something like "if the button is disabled, do nothing." Turning this requirement into code in a very straightforward manner, would result into this:
...
Even if our implementation will be correct, but I'd argue that *the specification is wrong* - as a user, I hate it when I click stuff and don't get any feedback (and as programmers you would start hating you too, if your users start contacting you with annoying questions), so I'd just throw an alert there.
...
Aside of the clarity, this line might be useful for debugging - imagine that in production some user clicks the button and nothing happens, where it should happen. This would mean that there is a bug somewhere in the code path but if it contains more than one `if`'s with no `else`-s then there would be no way for you to know which of them caused the deficiency.

Another instance of this issue is the situation where you have a bunch of cases that you handle in different ways and you don't really expect for something else to happen. 
...
```
This pattern is even more pervasive if you break the rule about conflating multiple conditions in one statements. My advice here is to do *something* - return a value, throw an exception, any exception, leave a breadcrumb that when the time comes would help you what went wrong.

Rule 4 - DO NOT encode data in `if` statements
===

No matter how punctual are we with our `if` statements, the logic of our programs ultimately depends on the business requirements, which often seem ugly when implemented directly. In these cases, you might help yourself by encoding some of those requirements in datastructures, or even a DSL, and only write code that deals with those datastructures. Suppose you have the following requirement.
...
```
A way to avoid this is to encode this logic as data, e.g. you can go back to the business analyst who wrote the requirements and tell them to create a table with all the conditions. In our example case, the table would look something like this:
...
And then you can use this table in your code and get the price by the following functional piece:
