...
Error handling with the strategy pattern
---

Let's see one more example of how we can utilize data structures, that has to do with error handling. We often have multiple conditions in which things can go wrong. The plain way to encode them is by having multiple consecutive `if`s:
...
Although it does not contradict rule 
