...
I'd say the problem is the problem with shared state. Namely that the possible states of a mutable object grow exponentially based on the functions which mutate the object. Those of you who have some programming experience are probably painfully aware what I mean but I will give an example for the rest of you. This is what I would call the "four button problem": Imagine a program which consists of just one button which modifies some internal state - to test the program we just press the button and check if the state is modified in the right way. Now, imagine the same problembut with two buttons instead of one and with shared state - you have two combinations of presses that you have to test - one is pressing "Button 1" and then "Button 2", another is pressing "Button 2" first. With three buttons the number of possible combinations is six, which is somehow managable, but four buttons it's 24 - which is too much. So if your program has more than 4 buttons which share a state it will be practically impossible to test.

The story so far 
===

In the beginning there were computations (and the desire to do more of them). Buildings, organizations, machines - none of these things can come to life without some amount of number crunching being done. What are they really? A computation is just the process in which one arithmetical term, is reduced to another, simpler term. It is as easy as **1 + 1 = 2**. 

Unfortunately we humans are not very smart creatures when it comes to computations, especially when it comes to bigger ones. We devised a method to overcome this weakness - the method consist of associating (or _binding_) terms which too ugly or too big for us to wrap our head around them, with symbols. This method always worked flawlessly - we just say **let A = some big term" and then we just type A whenever we need to type that term. This allows us to write the computation in fewer words, without losing any information - we know that A is equivalent to this big term, so we can always replace one for the other. This even allowed us to do part of our computation even before some of the values of this term are known, the practice of which changed mathematics for ever.
...
After some time some people decide to bring symbols back to programming. Their idea was to create a notation for writing computations on a computer and a program which converts that notation to a working computer code. In order to do that, they had to associate each mathematical operation that people invented to one or several instructions to the computer processor. They decided to associate the operation that we discussed earlier - the operation of _binding_ a simple symbol to a more complex term to the _assignment_ operation. Here is how the assignment operation works:

1. Compute the term right away (because the computer had not enough memory to hold all terms at once).
2. Put the result in a free location at the memory.

In this way the symbol that you use was actually interchangeble with the memory location. 
...
```
It is cool that we can return and pass programs arond, some of which are impure, but the functions which operate on them would still be pure.



Answers to the four button problem.
===
